...
Функция СЭ_ТабличныйДокументКарточкиНоменклатуры(Номенклатура, НастройкиВидимостиИЗаголовков, ДляПечати)
    ...
    Иначе
       //СЭ Ульяна 0016487 
       ОбластьШапка.Параметры.Наименование = Объект.СЭ_Наименование;
       //СЭ Ульяна 0016487
    #КонецВставки
	КонецЕсли;
	...
	Если Пользователи.РолиДоступны("СЭ_ПросмотрВсехКарточекНоменклатуры", Пользователи.ТекущийПользователь()) тогда
		Картинка = КартинкаФлагаПоЗначению(Объект.СЭ_РазрешеноИспользование);
		ОбластьШапка.Параметры.СЭ_РазрешеноИспользование = "";
		ОбластьШапка.Область("СЭ_РазрешеноИспользование").Картинка = Картинка;     
	Иначе
		ОбластьШапка.Параметры.СЭ_РазрешеноИспользование = "";
        ОбластьШапка.Область("СЭ_РазрешеноИспользование").Текст = "";  
		ОбластьШапка.Область("ТекстОбластьРазрешеноИспользование").Текст = "";
	КонецЕсли;
    //СЭ Ульяна 0016487   
	#КонецВставки
